...
class MetaWhatsAppSender(
 ...
 private fun send(to:String,payload:JsonObject):MetaSendResult { val request=Request.Builder().url("https://graph.facebook.com/$version/$phoneNumberId/messages").header("Authorization","Bearer $token").post(Json.encodeToString(payload).toRequestBody("application/json".toMediaType())).build(); client.newCall(request).execute().use { response -> val status=response.code; val body=response.body?.string().orEmpty(); if(status !in 200..299) { if(status in setOf(400,401,403,404)) { val error=metaError(body); throw NonRetryableWhatsAppException(status,error.code,error.type,error.message) }; throw IllegalStateException("Meta WhatsApp delivery failed with HTTP $status") }; return MetaSendResult(runCatching { Json.parseToJsonElement(body).jsonObject["messages"]?.jsonArray?.firstOrNull()?.jsonObject?.get("id")?.jsonPrimitive?.content }.getOrNull()) } }
 private fun metaError(body:String):MetaError { val error=runCatching { Json.parseToJsonElement(body).jsonObject["error"]?.jsonObject }.getOrNull(); return MetaError(error?.get("code")?.jsonPrimitive?.intOrNull,error?.get("type")?.jsonPrimitive?.contentOrNull,error?.get("message")?.jsonPrimitive?.contentOrNull) }
 private fun templatePayload(to:String,t:WhatsAppTemplateTransport)=buildJsonObject { put("messaging_product","whatsapp");put("to",to);put("type","template");put("template",buildJsonObject { put("name",t.name);put("language",buildJsonObject { put("code",t.languageCode) });put("components",buildJsonArray { add(buildJsonObject { put("type","body");put("parameters",buildJsonArray { t.parameters.forEach { p->add(buildJsonObject { put("type","text");put("text",p) }) } }) }); val button=t.buttonParameter ?: t.parameters.singleOrNull()?.takeIf { t.name==AUTHENTICATION_COPY_CODE_TEMPLATE }; button?.let { code->add(buildJsonObject { put("type","button");put("sub_type","url");put("index","0");put("parameters",buildJsonArray { add(buildJsonObject { put("type","text");put("text",code) }) }) }) } }) }) }
 private data class MetaError(val code:Int?,val type:String?,val message:String?)
 companion object { const val DEFAULT_GRAPH_API_VERSION="v22.0"; const val AUTHENTICATION_COPY_CODE_TEMPLATE="memgine_otp" }
}
