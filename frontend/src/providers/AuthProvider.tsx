...
import { authApi, type AuthSession, type OtpChallenge } from "@/src/data/api/auth-api";
...
type AuthContextValue = {
  ...
  passwordLogin: (phone: string, regionCode: string, password: string) => Promise<AuthSession>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    passwordLogin: async (phone, regionCode, password) => {
      const next = unwrap<AuthSession>(await authApi.passwordLogin(phone, regionCode, password));
      setSession(next);
      return next;
    },
    ...
    verifyOtp: async (challengeId, otp) => {
      const next = unwrap<AuthSession>(await authApi.verifyOtp(challengeId, otp));
      setSession(next);
      return next;
    },
    setPassword: async (password) => { unwrap(await authApi.setPassword(password)); },
    logout: async () => {
      const result = await authApi.logout();
      if (!result.success) throw new Error(result.error.message);
      setSession(null);
    },
    hasCapability: (capability, organizationId) => session?.access.some(
      (context) => context.capabilities.includes(capability) &&
