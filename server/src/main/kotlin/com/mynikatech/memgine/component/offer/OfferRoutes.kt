...
fun Route.offerRoutes(service: OfferService) {
    route("/organizations/{organizationId}/offers") {
        ...
        get("/{offerId}") {
            ...
            call.respond(ApiResponse.success(service.get(organizationId, offerId), call.callId))
        }
        get("/{offerId}/usage-rules") {
            val organizationId = requiredOfferId(call.parameters["organizationId"])
            val offerId = requiredOfferId(call.parameters["offerId"])
            call.respond(ApiResponse.success(service.rules(organizationId, offerId), call.callId))
        }
        post {
            val organizationId = requiredOfferId(call.parameters["organizationId"])
            call.respond(HttpStatusCode.Created,
                ApiResponse.success(service.save(organizationId, call.receive<OfferWriteDto>(), true), call.callId))
        }
        put("/{offerId}") {
            val organizationId = requiredOfferId(call.parameters["organizationId"])
            val offerId = requiredOfferId(call.parameters["offerId"])
        ...
        delete("/{offerId}") {
            val organizationId = requiredOfferId(call.parameters["organizationId"])
            val offerId = requiredOfferId(call.parameters["offerId"])
            call.respond(ApiResponse.success(service.delete(organizationId, offerId), call.callId))
        }
    }
}
