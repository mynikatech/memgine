...
class OrganizationUserService(private val sql:OrganizationUserSql,private val json:Json=Json{encodeDefaults=false;explicitNulls=false}) {
 ...
 fun upsert(organizationId:String,r:UpsertOrganizationUserRequest,actorUserId:String):JsonElement { if(r.userId.isBlank()||r.organizationUserId.isBlank()||r.userCode.isBlank()||r.firstName.isBlank()||r.primaryPhone.isBlank()||r.organizationUserTypeId.isBlank()) throw BadRequestException("Required organization user fields are missing"); return json.parseToJsonElement(sql.upsert(organizationId,json.encodeToString(r),actorUserId)) }
}
